...
class ProcessingThread(QThread):
    ...
    def __init__(self, model_path, pre_image_path, post_image_path, threshold=100):
        ...
        print(f"İşleme başlıyor - Hassasiyet eşiği: {self.threshold}")
    ...
    def run(self):
        try:
            ...
            change_map, overlay_map = change_detector.predict_changes(
                ...
                self.threshold  # Threshold değerini model fonksiyonuna geçir
            )
...
class SatelliteDetectionTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Eşik değerini gösteren ek etiket
        self.threshold_value_label = QLabel(f"{self.threshold}")
        self.threshold_value_label.setStyleSheet("color: #E1E1E6; font-weight: bold;")
        threshold_header.addWidget(self.threshold_value_label, alignment=Qt.AlignRight)
        ...
        # Büyütme butonu
        self.enlarge_btn = QPushButton("Görüntüyü Büyüt")
        self.enlarge_btn.setStyleSheet(BUTTON_STYLE)
        self.enlarge_btn.setEnabled(False)
        self.enlarge_btn.clicked.connect(self.show_enlarged_image)
        analysis_layout.addWidget(self.enlarge_btn)
    ...
    def update_threshold(self, value):
        ...
        if self.change_map is not None and self.overlay_map is not None:
            self.detect_changes(reprocess=True)
    ...
    def detect_changes(self, reprocess=False):
    ...
    def process_completed(self, change_map, overlay_map):
        ...
        self.enlarge_btn.setEnabled(True)
    ...
    def show_enlarged_image(self):
        """Analiz sonuçlarını büyük bir pencerede gösterir"""
        if self.change_map is None or self.overlay_map is None:
            QMessageBox.warning(self, "Uyarı", "Büyütülecek görüntü bulunamadı.")
            return
        
        # Hangi görüntünün gösterileceğini seç
        if self.overlay_checkbox.isChecked():
            image_to_show = self.overlay_map
            title = "Hasar Tespiti (Orijinal Görüntü Üzerinde)"
        else:
            image_to_show = self.change_map
            title = "Hasar Tespiti (Değişim Haritası)"
        
        # Büyütülmüş görüntü penceresi oluştur
        enlargeDialog = QDialog(self)
        enlargeDialog.setWindowTitle(title)
        enlargeDialog.setMinimumSize(800, 600)  # Daha büyük bir pencere boyutu
        enlargeDialog.setStyleSheet("background-color: #13111b;")
        
        # Kaydırılabilir alan
        scrollArea = QScrollArea(enlargeDialog)
        ...
        # Görüntü etiketi
        imageLabel = QLabel(contentWidget)
        imageLabel.setAlignment(Qt.AlignCenter)
        
        # Görüntüyü QPixmap'e dönüştür
        height, width, channel = image_to_show.shape
        bytes_per_line = 3 * width
        q_img = QImage(image_to_show.data, width, height, bytes_per_line, QImage.Format_RGB888)
        pixmap = QPixmap.fromImage(q_img)
        
        # Görüntüyü etiket üzerine yerleştir ve orijinal boyutunu koru
        imageLabel.setPixmap(pixmap)
        
        # Bilgilendirme metni
        infoText = ""
        if self.overlay_checkbox.isChecked():
            infoText = f"Afet sonrası görüntü üzerinde değişimler işaretlenmiştir. Hassasiyet: {self.threshold}"
        else:
            infoText = f"Değişim haritasında kırmızı alanlar değişimi göstermektedir. Hassasiyet: {self.threshold}"
        
        infoLabel = QLabel(infoText)
        infoLabel.setStyleSheet("color: #E1E1E6; font-size: 14px;")
        infoLabel.setWordWrap(True)
        infoLabel.setAlignment(Qt.AlignCenter)
        
        # Widget'a etiketleri ekle
        contentLayout.addWidget(infoLabel)
        contentLayout.addWidget(imageLabel, 1)
        
        # Kapatma butonu
        closeButton = QPushButton("Kapat")
        closeButton.setStyleSheet(BUTTON_STYLE)
        closeButton.clicked.connect(enlargeDialog.close)
        ...
        # Dialog'a scroll area'yı ekle
        dialogLayout = QVBoxLayout(enlargeDialog)
